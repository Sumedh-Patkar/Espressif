...
char* my_malloc(int requested_size)
{
	...
	/*Insert it into the linked list of free blocks in its appropriate position*/
...
void get_next_block(struct header **next_block_pointer,struct header *lptr)
{
  ...
  *next_block_pointer=(struct header *)((char *)(*next_block_pointer) + lptr->size + sizeof(struct header));
}
